...
class StorController extends Controller {
    ...
    public function __construct($m,$id = null,$name = null)
    {
        $this->auth($m,$id,$name);
    }

    public function chechRoute($m,$id = null,$name = null){
        switch($m){
            case 'index' :
                $this->index();
            break;
    ...
    public function search($name){
        $stor = new Stor();
        $data = $stor->where("name like '%$name%' or description like '%$name%'")->get();
        $rating = new Rating();
    ...
    public function auth($fun,$id = null,$name = null)
    {
        if(!empty($_GET['token'])){
            $token = $_GET['token'];
            $admin = new Admin();
            ...
            if($allowAdmin){
                $this->chechRoute($fun,$id,$name);
            }else{
                $this->unAuth();
                
            }
        }else{
            if($_GET['m'] == 'search' || $_GET['m'] == 'index' || $_GET['m'] == 'show'){
                $this->chechRoute($fun,$id,$name);
            }else{
                $this->unAuth();
            }
...
new StorController($method,$id,$name);